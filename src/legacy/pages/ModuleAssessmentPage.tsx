  ...
  return (
    ...
    >
      <div className="mx-auto w-full max-w-3xl">
        <div className="mb-8">
          <p className="text-xs font-medium uppercase leading-4 tracking-[0.18em] text-cyan-600 dark:text-cyan-400">Module assessment</p>
          <h1 className="mt-2 text-4xl font-bold leading-tight text-slate-950 dark:text-slate-50">{assessment?.title || "Module Assessment"}</h1>
          {moduleData?.title && <p className="mt-3 text-base font-normal leading-6 text-slate-600 dark:text-slate-400">{moduleData.title}</p>}
        </div>
        ...
        {!loading && assessment && assessment.questions.length > 0 ? (
          <>
            ...
            >
              {submitting ? "Submitting..." : selectedAnswers.includes("") ? "Answer all questions to submit" : "Submit assessment"}
            </button>
          ...
          !loading && <div className="py-12 text-center"><h2 className="text-lg font-semibold leading-7 text-slate-950 dark:text-slate-50">No assessment available</h2><p className="mt-2 text-sm font-normal leading-5 text-slate-500 dark:text-slate-400">There is no assessment configured for this module.</p></div>
        )}
        </div>
      </div>
    </motion.div>
