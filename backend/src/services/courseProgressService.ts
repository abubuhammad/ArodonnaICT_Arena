...
const normalizeProgress = (courseModules: any[], existingModules: unknown, completed: boolean): ProgressModule[] => {
  ...
  return courseModules.map((courseModule, moduleIndex) => {
    const storedModule: any = storedModules.find((item: any) =>
      String(item.module ?? item.moduleId) === String(courseModule.id)
    );
    const storedLessons = Array.isArray(storedModule?.lessonProgress)
      ? storedModule.lessonProgress
      : Array.isArray(storedModule?.lessons) ? storedModule.lessons : [];

    return {
      module: courseModule.id,
      status: completed
        ? ModuleStatus.PASSED
        : storedModule?.status || (moduleIndex === 0 ? ModuleStatus.NOT_STARTED : ModuleStatus.LOCKED),
      ...
      lessonProgress: (courseModule.lessons || []).map((lesson: any, lessonIndex: number) => {
        const storedLesson: any = storedLessons.find((item: any) =>
          String(item.lesson ?? item.lessonId) === String(lesson.id)
        );
        return {
          lesson: lesson.id,
          status: completed
            ? LessonStatus.PASSED
            : storedLesson?.status || (moduleIndex === 0 && lessonIndex === 0 ? LessonStatus.NOT_STARTED : LessonStatus.LOCKED),
...
async function loadProgress(enrollmentId: string) {
  const cp = await prisma.courseProgress.findUnique({ where: { enrollmentId } });
  if (!cp) throw new Error('Progress not found for this enrollment');
  return { cp, modules: progressModules(cp.modules || (cp.progress as any)?.modules) };
}
...
export async function startCourse(enrollmentId: string, userId: string) {
  ...
  if (state.enrollment.status !== 'IN_PROGRESS') {
    await prisma.enrollment.update({ where: { id: enrollmentId }, data: { status: 'IN_PROGRESS', startedAt: new Date() } });
  }
}
...
export async function completeLessonNoQuiz(enrollmentId: string, userId: string, lessonId: string) {
  ...
  if (lesson.hasQuiz) throw new Error('This lesson has a quiz and must be completed via quiz submission, not marked complete directly');
...
export function sanitizeQuestions(questions: any[]): any[] {
  const list = Array.isArray(questions) ? questions : [];
  return list.map(({ correctAnswer, correct, answer, ...rest }) => rest);
}

function autoGrade(questions: any[], answers: any[]): number {
...
export async function submitModuleAssessment(enrollmentId: string, userId: string, moduleId: string, answers: any[]) {
  ...
  const result = await completeModuleAssessment(enrollmentId, userId, moduleId, passed, score);
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (passed) {
      ...
      if (existing) {
        ...
        const template = enrollment.course.instructor.certificateTemplate || null;
        certificate = await tx.certificate.create({ data: { id, enrollmentId, serial: id, fileUrl, issuedAt: new Date(), metadata: { template } } });
...
export async function backfillMissingLessonProgress() { return { modified: 0 }; }
