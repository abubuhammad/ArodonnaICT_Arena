export interface ParsedMarkdownCourse {
  title: string;
  description: string;
  category: string;
  duration: string;
  ...
  modules: Array<{
    ...
    lessons: Array<{
      title: string;
      description: string;
      lessonType: "text-only" | "video" | "code";
      content: string;
      videoUrl: string;
      order: number;
    }>;
...
const valueAfterLabel = (line: string, label: string) =>
  line.slice(label.length).replace(/^\s*/, "").replace(/^:\s*/, "").trim();

const sectionText = (lines: string[], start: number, headingLevel: number) => {
  const values: string[] = [];
  for (let index = start; index < lines.length; index += 1) {
    if (new RegExp(`^#{1,${headingLevel}}\\s`).test(lines[index])) break;
...
export const parseCourseMarkdown = (markdown: string): ParsedMarkdownCourse => {
  const lines = markdown.replace(/^\uFEFF/, "").replace(/\r\n?/g, "\n").split("\n");
  const courseHeading = lines.find((line) => /^#\s+Course\s*:/i.test(line));
  if (!courseHeading) throw new Error("Markdown must start with a '# Course: Your title' heading.");
  ...
  let isFree = false;
  let thumbnail = "";
  const modules: ParsedMarkdownCourse["modules"] = [];
  let finalAssessment: ParsedMarkdownCourse["finalAssessment"] = null;
  const assessmentStart = lines.findIndex((line) => /^##\s+Final Assessment$/i.test(line.trim()));

  for (let index = 0; index < lines.length; index += 1) {
    const line = lines[index].trim();
    if (/^##\s+Description$/i.test(line)) description = sectionText(lines, index + 1, 2);
    if (/^##\s+Category$/i.test(line)) category = sectionText(lines, index + 1, 2);
    if (/^##\s+Duration$/i.test(line)) duration = sectionText(lines, index + 1, 2);
    if (/^##\s+Thumbnail$/i.test(line)) thumbnail = sectionText(lines, index + 1, 2);
  ...
  moduleMatches.forEach(([moduleIndex, moduleHeading], position) => {
    const nextModuleIndex = moduleMatches[position + 1]?.[0] ?? lines.length;
    ...
    const lessons = [...moduleLines.entries()]
      ...
      .map(([lessonIndex, lessonHeading], lessonPosition, lessonMatches) => {
        const nextLessonIndex = lessonMatches[lessonPosition + 1]?.[0] ?? moduleLines.length;
        ...
        const lessonType: "text-only" | "video" | "code" = rawType === "video" ? "video" : rawType === "code" ? "code" : "text-only";
        return {
  ...
  if (assessmentStart !== -1) {
    ...
    const questions = [...assessmentLines.entries()]
      ...
      .map(([questionIndex, questionHeading], position, questionMatches) => {
        const nextQuestionIndex = questionMatches[position + 1]?.[0] ?? assessmentLines.length;
    ...
    if (!finalAssessment.title || !finalAssessment.description || finalAssessment.timeLimit <= 0 || finalAssessment.passingScore < 0 || finalAssessment.passingScore > 100 || finalAssessment.questions.length === 0 || finalAssessment.questions.some((question) => !question.question || question.options.length < 2 || !question.correctAnswer || !question.options.includes(question.correctAnswer))) {
